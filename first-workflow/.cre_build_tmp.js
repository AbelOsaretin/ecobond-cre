...
var writeDataOnchain = (runtime2) => {
  ...
  const offchainValue = runtime2.runInNodeMode(fetchAndParseData, ignore())().result();
  runtime2.log(`Successfully fetched offchain value: ${JSON.stringify(offchainValue, null, 2)}`);
  const reportData = encodeAbiParameters(parseAbiParameters("((uint8,uint8),uint256,string)[]"), [
    offchainValue.data.map((item) => [
      [item.creditQuality, item.greenImpact],
      BigInt(item.deviceId),
      ""
    ])
  ]);
...
var fetchAndParseData = (nodeRuntime) => {
