...
var fetchAndParse = (sendRequester, config) => {
  const req = {
    url: config.apiUrl,
    method: "GET"
  };
  ...
  return {
    ...
    items: externalResp.data.map((item) => ({
      id: item.id,
      title: item.title,
      description: item.description,
      createdAt: new Date(item.createdAt)
    })),
    count: externalResp.count
  };
};
var postData = (sendRequester, config) => {
  const dataToSend = {
    title: "Hello World!",
    description: "My data is live."
  };
  ...
  const resp = sendRequester.sendRequest(req).result();
  console.log("Received response with status:", resp.statusCode);
  if (!ok(resp)) {
...
var writeDataOnchain = (runtime2) => {
  ...
  runtime2.log(`Selected network: ${network248}`);
  if (!network248) {
  ...
  const reportData = encodeAbiParameters(parseAbiParameters("((uint8,uint8),uint256,string)[]"), [
    [
      [
        [84, 91],
        1n,
...
var onCronTriggerGet = (runtime2) => {
  const httpClient = new ClientCapability2;
  const result = httpClient.sendRequest(runtime2, fetchAndParse, ignore())(runtime2.config).result();
  runtime2.log("Workflow triggered.");
  return result;
};
var onCronTriggerPost = (runtime2) => {
  const httpClient = new ClientCapability2;
  const result = httpClient.sendRequest(runtime2, postData, ignore())(runtime2.config).result();
  runtime2.log("Workflow triggered.");
  return result;
};
var initWorkflow = (config) => {
  ...
  return [
    handler(cron.trigger({ schedule: config.schedule }), onCronTriggerGet),
    handler(cron.trigger({ schedule: config.schedule }), onCronTriggerPost),
